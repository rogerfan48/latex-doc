...
%   homework — 標題左右對齊（標題+課名 左，日期+姓名 右），頁首左=\docheader（預設 subtitle）右=姓名
...
% \section{標題}        — 第一層（自動編號 1. 2. 3.）
% \subsection{標題}     — 第二層（1.1 1.2）
% \subsubsection{標題}  — 第三層（1.1.1）
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\linewidth]{head}
    \caption{縮到 60\% 頁寬。}
    \label{fig:demo-60}
\end{figure}
